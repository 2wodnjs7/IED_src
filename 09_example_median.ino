...
// get a distance reading from the USS
  ...
  addq(dist_raw);
  array_copy(queue, median_array);
  median = search_median(median_array);
...
float search_median(float a[]) {
  ...
  if(N%2==0) median = (a[(N-1)/2]+a[N/2])/2;
  else median = a[N/2];
  return median;
}
